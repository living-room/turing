module.exports = async room => {
  ...
  room.subscribe([
    ...
    `$mac got input event type $type with code $code and value $value`
  ], ({ assertions }) => {
    assertions.forEach(({code, type, value}) => {
      if (code.value !== 4) return
      if (type.value !== 4) return
      console.log(buf.join(''))
      room.retract(`whiteboard: draw small text "${buf.join(' ')}" at (0.05, 0.20)`)
      buf.push(value.value - 458700)
      room.assert(`whiteboard: draw small text "${buf.join(' ')}" at (0.05, 0.20)`)
    })
  })
