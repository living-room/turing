...
function moveMouse(vec) {
    if (lastMousePositions.length > 9) {
        ...
        for (let i = 0; i < 10; i += 1) {
            room.assert(`mouse${alphabet[i]} is a (${255 - i * 10}, ${255 - i * 10}, ${255 - i * 10}) circle at (${lastMousePositions[i].x}, ${lastMousePositions[i].y}) with radius ${30 - i * 2}`);
            console.log(`mouse${alphabet[i]} is a (${255 - i * 10}, ${255 - i * 10}, ${255 - i * 10}) circle at (${lastMousePositions[i].x}, ${lastMousePositions[i].y}) with radius ${30 - i * 2}`);
